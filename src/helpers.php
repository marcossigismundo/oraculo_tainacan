...
function format_tainacan_item( \Tainacan\Entities\Item $item ): array {
	...
	foreach ( $item_metadata as $meta ) {
		...
		if ( ! empty( $value ) ) {
			$metadata[ $metadatum->get_slug() ] = array(
 ...
 * @return string
 */
function format_item_for_indexing( array $item, array $fields = array( 'title', 'description' ) ): string {
	$parts = array();
...
function sanitize_appearance( array $appearance ): array {
	return array(
		...
		'chat_position'   => in_array( $appearance['chat_position'] ?? '', array( 'bottom-right', 'bottom-left', 'top-right', 'top-left' ) )
 ...
 */
function oraculo_tainacan_render_template( string $template, array $vars = array() ): void {
...
function get_collections_indexing_status(): array {
	...
	foreach ( $collections as $collection ) {
		...
		elseif ( in_array( $job_status, array( 'running', 'processing' ) ) ) {
