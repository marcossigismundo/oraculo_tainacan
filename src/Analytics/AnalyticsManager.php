...
class AnalyticsManager {
	...
	public function get_top_searches( string $period = 'month', int $limit = 10 ): array {
		...
		$args = array_merge( $where_args, array( $limit ) );
	...
	private function get_date_where( string $period ): array {
		switch ( $period ) {
			...
			case 'yesterday':
				return array( 'DATE(created_at) = DATE_SUB(CURDATE(), INTERVAL %d DAY)', array( 1 ) );
			case 'week':
	...
	public function export( string $period = 'month', string $format = 'json' ): string {
		...
		if ( $format === 'csv' ) {
			...
			fputcsv( $output, array( 'Top Buscas' ) );
			fputcsv( $output, array( 'Query', 'Count', 'Avg Results' ) );
			foreach ( $data['top_searches'] as $row ) {
				fputcsv( $output, array( $row['query_text'], $row['count'], $row['avg_results'] ) );
			}
