...
class MultimodalSearch {
	 ...
	 */
	public function index_image( int $attachment_id, array $metadata = array() ) {
	 ...
	 * @return array|WP_Error
	 */
	public function search_by_image( string $image_path, array $options = array() ) {
	...
	private function call_claude_vision( string $prompt, array $image_data, string $model ) {
		...
		if ( $image_data['type'] === 'url' ) {
			...
			$image_response = wp_remote_get( $image_data['url'], array( 'timeout' => 30 ) );
			if ( is_wp_error( $image_response ) ) {
				return $image_response;
			}
			$content = wp_remote_retrieve_body( $image_response );
			if ( empty( $content ) ) {
				return new WP_Error( 'image_download_failed', __( 'Não foi possível baixar a imagem informada.', 'oraculo-tainacan' ) );
			}
			$image_source = array(
				'type'       => 'base64',
