...
class WebhooksManager {
	...
	public function update( int $id, array $data ): bool {
		...
		if ( isset( $data['events'] ) ) {
			$update_data['events'] = wp_json_encode( is_array( $data['events'] ) ? $data['events'] : array( $data['events'] ) );
			$formats[]             = '%s';
		}
	...
	public function delete( int $id ): bool {
		...
		$result = $wpdb->delete( $this->table_name, array( 'id' => $id ), array( '%d' ) ) !== false;
		if ( $result ) {
			\Oraculo_Tainacan\oraculo_tainacan_flush_cache();
		}
		return $result;
	 ...
	 * @return array
	 */
	public function get_webhooks( array $filters = array() ): array {
		global $wpdb;
	 ...
	 */
	public function trigger( string $event, array $payload = array() ): array {
	 ...
	 * @return bool
	 */
	public function send_email_notification( string $subject, string $message, array $options = array() ): bool {
	 ...
	 * @return bool
	 */
	public function send_slack_notification( string $message, string $webhook_url, array $options = array() ): bool {
		$payload = array(
			'text' => $message,
		);
	 ...
	 * @return bool
	 */
	public function send_discord_notification( string $message, string $webhook_url, array $options = array() ): bool {
		$payload = array(
			'content' => $message,
		);
		...
		return ! is_wp_error( $response ) && in_array( wp_remote_retrieve_response_code( $response ), array( 200, 204 ) );
	}
