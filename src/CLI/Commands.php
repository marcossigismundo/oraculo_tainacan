...
class Commands extends WP_CLI_Command {
    ...
    public function status($args, $assoc_args) {
        ...
        WP_CLI\Utils\format_items($format, $data, ['ID', 'Nome', 'Total', 'Indexados', 'Porcentagem']);
    }
    ...
    public function search($args, $assoc_args) {
        ...
        if ($no_ai) {
            ...
            $result = $search->search($query, $collections, ['max_results' => $limit]);
        }
        ...
        $items = $no_ai ? $result : ($result['items'] ?? []);
        ...
        if ($format === 'json') {
            ...
            WP_CLI\Utils\format_items('table', $table_data, ['ID', 'Título', 'Coleção', 'Similaridade']);
        }
    ...
    public function providers($args, $assoc_args) {
        ...
        WP_CLI\Utils\format_items($format, $data, ['ID', 'Nome', 'Configurado', 'Embeddings', 'Streaming']);
    }
    ...
    public function optimize($args, $assoc_args) {
        ...
        foreach ($tables as $table) {
            // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared,WordPress.DB.DirectDatabaseQuery.DirectQuery -- Plugin-owned table; CLI maintenance command with no WP API equivalent.
