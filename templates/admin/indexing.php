                    ...
                    <td>
                        ...
                        <select name="embedding_provider">
                            <?php foreach ($providers as $provider) :
                                if (!$provider['supports_embeddings']) continue;
                            ?>
                                <option value="<?php echo esc_attr($provider['id']); ?>"
                                        <?php selected($current, $provider['id']); ?>>
                                    <?php echo esc_html($provider['name']); ?>
                                    <?php if (!$provider['is_configured']) echo ' (Não configurado)'; ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </td>
...
jQuery(document).ready(function($) {
    ...
    function addLog(message, type) {
        ...
        logContainer.append('<div class="oraculo-log-entry ' + (type || '') + '">[' + time + '] ' + message + '</div>');
        logContainer.scrollTop(logContainer[0].scrollHeight);
    }

    // Indexar coleção individual - indexação síncrona
    $('.oraculo-btn-index').on('click', function() {
        var btn = $(this);
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    if (data.failed_items > 0) {
                        addLog('⚠️ ' + data.failed_items + ' itens falharam na indexação.', 'error');
    ...
    $('.oraculo-btn-clear').on('click', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    addLog('Vetores da coleção #' + collectionId + ' removidos.', 'success');
    ...
    $('#oraculo-index-all').on('click', function() {
        ...
        addLog('Iniciando indexação de todas as coleções...', 'info');
    ...
    $('#oraculo-clear-all').on('click', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    addLog('Todos os vetores foram removidos.', 'success');
                    location.reload();
                } else {
                    addLog('Erro: ' + response.data.message, 'error');
                }
            }
        });
    });
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    addLog('Erro na otimização: ' + response.data.message, 'error');
                }
            }
        });
    });
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    addLog('Erro ao salvar: ' + response.data.message, 'error');
                }
            }
        });
    });
